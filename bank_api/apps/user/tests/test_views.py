...
class UserTestCase(TestCase):
    ...
    def test_user_login(self):
        request = self.client_user.post(
            "/user/login/", data={"username": "foo", "password": "bar"}
        )
        self.assertEqual(request.status_code, 200)

    def test_user_login_wrong_passw(self):
        request = self.client_user.post(
            "/user/login/", data={"username": "foo", "password": "bar1"}
        )
        self.assertEqual(request.status_code, 403)

    def test_user_login_wrong_username(self):
        request = self.client_user.post(
            "/user/login/", data={"username": "foo1", "password": "bar"}
        )
        self.assertEqual(request.status_code, 403)
